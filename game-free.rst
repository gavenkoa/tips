...
Related packages::
  ...
  $ aptitude search '?tag(chess)'

Chess engines::

  $ sudo apt-get install crafty toga2 fruit glaurung gnuchess stockfish

GUI::

 * ``cutechess`` - commandline and graphical interface for chess engine vs.
   engine matches
 * ``xboard`` - X Window System Chess Board

Check engine strength on http://www.computerchess.org.uk/ccrl/
...
Commander Genius is a software piece that interprets the Commander Keen Invasion
of the Vorticons and Galaxy series. As fans and developers, we are, we try to
implement new features, improve the gameplay, and give players an experience
that feels like playing the original game but a bit more refreshing.
